...
namespace DecryptCocos2dAsset
{
    class Program
    {
        ...
        {
            //7905A356 625392BA 6F09097A 1E4EE4ED
            SetKey(0x7905A356, 0x625392BA, 0x6F09097A, 0x1E4EE4ED);
            InitializeKey();
            //ExportKey();

            foreach (string file in Directory.EnumerateFiles(".", "*.ccz"))
            {
                ...
                try
                {
                    ...
                    File.WriteAllBytes(file.Replace(".pvr.ccz", "") + "-decrypt.pvr", bytes);
                    Console.WriteLine("decrypted " + file);
                }
                catch (Exception err)
                {
                    Console.WriteLine("Error on extracting " + err);
